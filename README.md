...
| `/get <path>` | **Pull a server file back to your phone.** Uploads the given file to Telegram as a tappable document — tap to preview / download / share. Useful when Claude writes a report, log, or code file on the server and you want to read or forward it from your phone. Paths can be absolute (`/home/you/report.md`), `~`-prefixed (`~/report.md`), or relative (resolved against the active pane's `pane_current_path` — so `/get report.md` works from inside a topic). Bare `/get` prompts via ForceReply. Cap: 50 MB (Telegram sendDocument limit). |
| `/new [dir]` | Spawn a fresh Claude pane in a **new detached tmux session** named `claude-<basename>` (collision-suffixed: `-2`, `-3`, …). With a directory arg, spawns immediately (accepts `~/foo` or absolute paths). **Bare `/new` prompts via ForceReply** (misclick-safe from the ☰ Menu) — reply with a dir, or `.` to use `$HOME`. The new session shows up in `tmux ls`; attach anytime with `tmux attach -t <name>` (or `switch-client` from inside tmux). Launches `TELE_CLAUDE=1 claude --dangerously-skip-permissions` in the pane (override via `TELE_CLAUDE_NEW_LAUNCH_CMD` if you want a different binary or flags), auto-subscribes it, and makes it the active pane — the next Telegram message goes there without `/use`, no attach needed. Detached means your current terminal never gets yanked. |
| `/resume [dir]` | **Resume a previous Claude session from your phone.** Lists up to 12 most-recent sessions from `~/.claude/projects/*/`, newest first. Each button shows `<project basename> · <age> · <first-prompt preview>`. Tap one → bot spawns a fresh pane in that session's recorded `cwd` running `<launch-cmd> --resume <session-id>`, auto-subscribes + activates it (same flow as `/new`, including ForceReply-free + waits for the TUI to be ready). With `<dir>` (absolute, `~`-prefixed, or relative), narrows the picker to sessions whose `cwd` sits under that directory — handy when several repos share a basename. Past 12 sessions you currently need `/resume <dir>` to narrow. |
